...
class GFWebAppSupabaseUser extends BaseAuthUser {
  GFWebAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> gFWebAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = GFWebAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
